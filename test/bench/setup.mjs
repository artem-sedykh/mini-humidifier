...
// The starting state a fixture needs, made the way a user would make it: by
// calling the service.
//
// The fixtures carry no state topics for anything the card writes, so Home
// Assistant is optimistic about them - a press moves the entity with nothing on
// the other end to echo it back, which is the whole reason a scenario can
// assert on a press at all. The cost is where they start: an optimistic MQTT
// humidifier comes up `unknown`, the card reads that as unavailable, and every
// control it would have drawn is not there to be pressed.
//
// So the manifest may give an entity a `start` list, and each entry is one
// service call with the fixture's own id filled in.
const start = async (token, entities, ids) => {
  let called = 0;

  for (const entity of entities) {
    for (const step of entity.start || []) {
      await request(
        `/api/services/${step.domain}/${step.service}`,
        { entity_id: ids[entity.id], ...(step.data || {}) },
        token,
      );
      called += 1;
    }
  }
  if (called) await wait(1000);
...
// build between releases. See #275.
//
...
// not. Measured in one browser: a reload with nothing changed reports
// `transferSize: 0`, and a reload after a deploy fetches the whole 95KB again.
...
const setupLovelace = async (token, manifest, ids) => {
  ...
  // load. This bench inherited its config from the sister card and had been
  // carrying `mini-climate-card-bundle.js` ever since - which the scenarios'
  // console assertion only started catching once these urls began changing
  // between runs.
...
export const prepare = async () => {
  ...
  const started = await start(token, manifest.entities, ids);
  await setupLovelace(token, manifest, ids);

  return { manifest, tokens, ids, broker, forgotten, started, dashboard: `${BASE}/${DASHBOARD}/0` };
};
