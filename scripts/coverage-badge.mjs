...
// browser layer covers the components and the model configurations and reports
// separately, so a percentage that claimed to speak for both would be a
// number nobody could act on. See the comments on `coverage.exclude`.
...
// nothing downstream would notice - `check:bundle` reported "ok" on an empty
// list of models for months (#185).
const read = () => {
  let raw;
  try {
