...
//
// Nothing reads the prose. Paths in AGENTS.md ended in `.js` from the
// TypeScript migration and nothing noticed - they are fixed here rather than by
// hand every time - and no check in this repository had an opinion about any of
// it. `mkdocs build --strict` does not cover this: it validates the links
// between pages of the site, not the paths a sentence names, and it never sees
// AGENTS.md or CONTRIBUTING.md at all.
//
//   npm run check:docs
//
...
const IGNORED = new Map([
  ['dist/mini-humidifier-bundle.js', 'the build output, absent from a clean tree'],
  ['mini-humidifier-bundle.js', 'the build output, absent from a clean tree'],
  ['config/www/mini-humidifier-bundle.js', "the reader's built bundle, outside this tree"],
  ...
  ['/local/mini-humidifier-bundle.js', "a resource URL on the reader's Home Assistant"],
  [
    '/local/mini-humidifier/dist/mini-humidifier-bundle.js',
    "a resource URL on the reader's Home Assistant",
  ],
  ['Node.js', 'the runtime, not a path in this tree'],
...
const SKIPPED_DIRS = new Set(['node_modules', 'dist', '.git', 'site', 'coverage', '__pycache__']);
